...
public class CookieInfo {
     ...
     * 添加登陆时的 cookie
     */
    public static void addLoginCookie(CustomerInfo customer, HttpServletRequest request, HttpServletResponse response) {
        ...
        String toEnc = REDIS_COOKIE_CUSTOMER_PREFIX + System.currentTimeMillis() + StringUtil.COLON + customer.getId();
    ...
    public static void removeLoginCookie(HttpServletRequest request, HttpServletResponse response) {
        ...
        // 删除相关cookie
        Cookie[] cookies = request.getCookies();
        String customerId = StringUtil.EMPTY;
        if (cookies != null) {
            for (Cookie cookie : cookies) {
                if (LOGIN_UUID_COOKIE_NAME.equals(cookie.getName())
    ...
    public static String getCustomerIdByCookie(HttpServletRequest request) {
        ...
        if (StringUtil.isNotBlank(customerId)) {
            ...
            customerId = tmp.substring(tmp.lastIndexOf(StringUtil.COLON) + 1);
        }
        return customerId;
    }
